...
//			ui_g_Dynamic_Right_down_Leg->end_y = 741 + 99 * (0.55 * cos(UI_State.Joint_Angle * 2.0f*PI/360.0f) - 0.84 * sin(UI_State.Joint_Angle * 2.0f*PI/360.0f));
            ...
            /*===| Distance |===*/
            // Point_Pitch y+-;  Base_Point1\Base_Point2 x+-; CoastLine1\CoastLine2 x+-
            // x Point_Pitch = x Base_Pitch
            // y Base_Point     = y Base_Pitch
            // AO = 300 * tanf(UI_State.Pitch) 所有点位均围绕AO展开计算
            
            /*===| Pitch_BasePoint |===*/
            ui_g_Dynamic_Point_Pitch->start_x = 959;
            ui_g_Dynamic_Point_Pitch->start_y = 540 + 300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f);
            
            /*===| CoastLine_BasePoint |===*/
            ui_g_Dynamic_Base_Point1->start_x = 959 - 434 * 300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f) / (300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f) + 300); 
            ui_g_Dynamic_Base_Point1->start_y = 540;
            
            ui_g_Dynamic_Base_Point2->start_x = 959 + 434 * 300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f) / (300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f) + 300); 
            ui_g_Dynamic_Base_Point2->start_y = 540;
            
            /*===| CoastLine |===*/
            ui_g_Dynamic_CoastLine1->end_x      = ui_g_Dynamic_CoastLine1->start_x + 260 * 434 / (300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f) + 300);
            ui_g_Dynamic_CoastLine1->end_y      = 500;
            
            ui_g_Dynamic_CoastLine2->end_x      = ui_g_Dynamic_CoastLine2->start_x - 260 * 434 / (300 * tanf(UI_State.Pitch * 2.0f*PI/360.0f) + 300); 
            ui_g_Dynamic_CoastLine2->end_y      = 500;
